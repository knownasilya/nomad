...
async function checkin() {
  ...
  if (isEnabled == 1) {
    try {
      ...
      if (
        Date.now() - (pingData.lastPingTime || 0) >
        ANALYTICS_CHECKIN_INTERVAL
      ) {
      ...
      (res) => {
        if (res.statusCode === 204) {
          ...
          res.pipe(
            concat((body) =>
              logger.info(
                `Ping failed: ${res.statusCode} ${JSON.stringify(body)}`
              )
            )
          );
          reject();
...
async function readPingData() {
  var data = await jetpack.readAsync(
    path.join(app.getPath('userData'), ANALYTICS_DATA_FILE),
    'json'
  );
  return (
    data || { lastPingTime: 0, id: crypto.randomBytes(32).toString('hex') }
  );
}
