...
import {
  ...
  shouldShowMenuItem as shouldShowContextMenuItem,
...
export async function setup() {
  ...
  ipcMain.on('page-prompt-dialog', async (e, message, def) => {
    var wc = e.sender;
    var res = await modals
      .create(e.sender, 'prompt', { message, default: def })
      .catch((e) => false);
    wc.focus();
  ...
  ipcMain.on('resize-hackfix', (e, message) => {
    ...
    if (win) {
      win.webContents.executeJavaScript(
        `if (window.forceUpdateDragRegions) { window.forceUpdateDragRegions() }; undefined`
      );
    }
  });
...
export async function listBuiltinFavicons({ filter, offset, limit } = {}) {
  ...
  var items = (await dir.listAsync()).filter((filename) => {
...
export async function uploadFavicon() {
  let favicon = dialog.showOpenDialogSync({
    ...
    filters: [{ name: 'Images', extensions: ['png', 'ico', 'jpg'] }],
    properties: ['openFile'],
  });
  ...
  if (enabled) {
    ...
    _windowDragInterval = setInterval(() => {
      ...
      if (
        newPt.x < bounds.x ||
        newPt.y < bounds.y ||
        newPt.x > bounds.x + bounds.width ||
        newPt.y > bounds.y + bounds.height
      ) {
...
async function executeJavaScriptInPage(js) {
  return getSenderTab(this.sender)
