...
var datPath;
/** @type string - path to the dat folder */
...
export function listLegacyArchives() {
  return db.all(
    `SELECT archives.*, archives_meta.title from archives JOIN archives_meta ON archives_meta.key = archives.key WHERE archives.isSaved = 1`
  );
}
