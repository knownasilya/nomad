...
export async function addSite(profileId, url, opts) {
  ...
  try {
    ...
    if (!site) {
      ...
      await db.run(
        'INSERT INTO watchlist (profileId, url, description, seedWhenResolved, createdAt) VALUES (?, ?, ?, ?, ?);',
        [profileId, url, opts.description, opts.seedWhenResolved, ts]
      );
    }
  } finally {
    release();
...
export async function updateWatchlist(profileId, site) {
  ...
  try {
    await db.run(
      `UPDATE watchlist SET seedWhenResolved = ?, resolved = ?, updatedAt = ?
    WHERE profileId = ? AND url = ?`,
      [site.seedWhenResolved, site.resolved, updatedAt, profileId, site.url]
    );
  } finally {
    release();
  }
