...
export async function query(root, opts) {
  ...
  if (
    !(
      ...
      (Array.isArray(opts.path) &&
        opts.path.every((v) => typeof v === 'string'))
    )
  ) {
    throw new Error(
      'The `path` parameter must be a string or array of strings'
    );
  }
  ...
  await chunkMapAsync(candidates, 100, async (item) => {
    ...
    if (
      opts.mount &&
      (type !== 'mount' || stat.mount.key.toString('hex') !== opts.mount)
    )
      return;
    if (opts.metadata) {
      let metaMatch = true;
    ...
    results.push({
      ...
      mount:
        type === 'mount'
          ? await keyToUrl(stat.mount.key.toString('hex'))
          : undefined,
      origin: {
        path: originPath,
        drive: originDrive,
  ...
  if (opts.sort === 'name') {
    results.sort((a, b) =>
      opts.reverse
        ? basename(b.path)
            .toLowerCase()
            .localeCompare(basename(a.path).toLowerCase())
        : basename(a.path)
            .toLowerCase()
            .localeCompare(basename(b.path).toLowerCase())
    );
  } else if (opts.sort === 'mtime') {
    results.sort((a, b) =>
      opts.reverse ? b.stat.mtime - a.stat.mtime : a.stat.mtime - b.stat.mtime
    );
  } else if (opts.sort === 'ctime') {
    results.sort((a, b) =>
      opts.reverse ? b.stat.ctime - a.stat.ctime : a.stat.ctime - b.stat.ctime
    );
  }
      ...
      var workingPaths = [
        {
          path: '/',
          originPath: '/',
          originDriveKey: root.key.toString('hex'),
          stat: undefined,
        },
      ];
        ...
        if (op[0] === 'push') {
          ...
          newWorkingPaths = await Promise.all(
            workingPaths.map(async (workingPath) => {
              ...
              let statMountKey =
                stat.mount && stat.mount.key
                  ? stat.mount.key.toString('hex')
                  : undefined;
              let isNewMount =
                statMountKey && statMountKey !== workingPath.originDriveKey;
              return {
                path: statpath,
                originPath: isNewMount
                  ? '/'
                  : joinPath(workingPath.originPath, op[1]),
                originDriveKey: isNewMount
                  ? statMountKey
                  : workingPath.originDriveKey,
                stat: stat,
              };
            })
          );
          ...
          for (let workingPath of workingPaths) {
            let items = await auditLog.record(
              ...
              () =>
                root.pda
                  .readdir(workingPath.path, { includeStats: true })
                  .catch((e) => [])
            );
            for (let item of items) {
              ...
              if (re.test(item.name)) {
                let statMountKey =
                  item.stat.mount && item.stat.mount.key
                    ? item.stat.mount.key.toString('hex')
                    : undefined;
                let isNewMount =
                  statMountKey && statMountKey !== workingPath.originDriveKey;
                newWorkingPaths.push({
                  ...
                  originPath: isNewMount
                    ? '/'
                    : joinPath(workingPath.originPath, item.name),
                  originDriveKey: isNewMount
                    ? item.stat.mount.key.toString('hex')
                    : workingPath.originDriveKey,
                  stat: item.stat,
