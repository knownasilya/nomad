...
async function _writeMeta(sess, path, { executable = false } = {}) {
  ...
  try {
    ...
    if (cur && cur.value && cur.value.metadata && cur.value.metadata.ctime) ctime = cur.value.metadata.ctime
  } catch {}
...
export async function buildPutInlineOp(sess, path, data, opts = {}) {
  ...
  return { op: 'put', path, metadata: await _writeMeta(sess, path, opts), value: b4a.toString(bytes, 'base64') }
}
...
export async function readJson(sess, path) {
  ...
  if (!buf) return null
  try { return JSON.parse(b4a.toString(buf)) } catch { return null }
}
...
export async function listRecords(sess, prefix) {
  ...
  for await (const node of sess.drive.createReadStream(opts)) {
    out.push({ path: typeof node.key === 'string' ? node.key : b4a.toString(node.key), record: node.value })
  }
  return out
}
...
export function recordByteLength(record) { return _content.contentLength(record) }
...
export function viewEmpty(sess) {
  try {
    return !sess || !sess.base || !sess.base.view || !sess.base.view.core || sess.base.view.core.length === 0
  } catch {
    return true
  }
}
