...
async function _saveInviteKeys() {
  try {
    await settingsDb.set(INVITE_KEYS_SETTING, JSON.stringify(invitePublicKeys.map((k) => b4a.toString(k, 'hex'))))
  } catch {}
}
...
async function _processCandidate(candidate, resolve) {
  ...
  if (!_openCandidate(candidate)) {
    logger.error('Could not open pairing candidate with any known invite key', { invites: invitePublicKeys.length })
    return resolve()
  }
  ...
  if (approved[data.key]) {
    ...
    if (sess) {
      try { candidate.confirm({ key: sess.base.key, encryptionKey: sess.base.encryptionKey }) } catch (e) {
  ...
  events.emit('request', { deviceKey: data.key, name: pending[data.key].name, platform: pending[data.key].platform })
}
