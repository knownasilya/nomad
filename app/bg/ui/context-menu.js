  ...
  app.on('web-contents-created', (e, webContents) => {
    webContents.on('context-menu', async (e, props) => {
      ...
      const downloadPrompt = (field, ext) => async (item, win) => {
        ...
        if (filePath) {
          download(win, webContents, props[field], { saveAs: filePath });
        }
      };
      ...
      if (props.linkURL) {
        menuItems.push({
          ...
          click: (item, win) =>
            tabManager.create(win, props.linkURL, {
              setActiveBySettings: true,
              adjacentActive: true,
            }),
        });
        menuItems.push({
        ...
        menuItems.push({
          label: 'Save Link As...',
          click: downloadPrompt('linkURL', '.html'),
        });
      ...
      if (props.mediaType == 'image') {
        menuItems.push({
          label: 'Save Image As...',
          click: downloadPrompt('srcURL'),
        });
        menuItems.push({
          label: 'Copy Image',
          click: () => webContents.copyImageAt(props.x, props.y),
        });
        menuItems.push({
      ...
      if (props.isMisspelled !== '' && props.isEditable) {
        ...
        if (spellingSuggestions) {
          for (let i in spellingSuggestions) {
            menuItems.push({
              label: spellingSuggestions[i],
              click: (item, win) =>
                webContents.replaceMisspelling(item.label, {
                  adjacentActive: true,
                }),
            });
          }
      ...
      menuItems.push({ type: 'separator' });
      menuItems.push(
        createMenuItem('inspect-element', {
          webContents,
          tab: targetTab,
          x: props.x,
          y: props.y,
        ...
        submenu: [
          {
            ...
            click() {
              ...
              if (tab && pane) {
                tab.detachPane(pane);
                tabManager.create(tab.browserWindow, null, {
                  setActive: true,
                  initialPanes: [pane],
                });
              }
