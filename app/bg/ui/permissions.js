...
export function requestPermission(permission, webContents, opts) {
  return new Promise((resolve, reject) =>
    onPermissionRequestHandler(webContents, permission, resolve, opts)
  );
}
...
export async function checkLabsPerm({ perm, labApi, apiDocsUrl, sender }) {
  ...
  if (urlp.protocol === 'nomad:') return true;
  if (urlp.protocol === 'hyper:') {
    ...
    if (!isOptedIn) {
      throw new PermissionsError(
        `You must include "${labApi}" in your index.json experimental.apis list. See ${apiDocsUrl} for more information.`
      );
    }
...
async function onPermissionRequestHandler(webContents, permission, cb, opts) {
  ...
  var decision = await permPromptSubwindow.create(win, tab, {
    permission,
    url,
    opts,
  });
