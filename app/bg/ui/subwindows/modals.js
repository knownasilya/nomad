...
export async function create(webContents, modalName, params = {}) {
  ...
  try {
    result = await view.webContents.executeJavaScript(
      `runModal("${modalName}", ${JSON.stringify(params)})`
    );
  } catch (e) {
    err = e;
  }
...
function setBounds(view, parentWindow, { width, height } = {}) {
  ...
  view.currentBounds = view.currentBounds || {
    width: undefined,
    height: undefined,
  };
  view.currentBounds.width = Math.min(
    width || view.currentBounds.width || getDefaultWidth(view),
    parentBounds.width - 20
  );
  view.currentBounds.height = Math.min(
    height || view.currentBounds.height || getDefaultHeight(view),
