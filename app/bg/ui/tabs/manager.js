    ...
    for (let stackSnapshot of stacks) {
      ...
      for (let paneSnapshot of panes) {
        ...
        if (stack) {
          ...
          this.layout.addPaneToStack(stack, pane, {
            after,
            layoutHeight: paneSnapshot.layoutHeight,
            noRebalance: true,
          });
        } else {
          ...
          this.layout.addPane(pane, {
            after,
            layoutWidth: stackSnapshot.layoutWidth,
            layoutHeight: paneSnapshot.layoutHeight,
            noRebalance: true,
          });
...
export async function loadPins(win) {
  ...
  try {
    JSON.parse(json).forEach((url) => create(win, url, { isPinned: true }));
  } catch (e) {}
...
rpc.exportAPI('background-process-views', viewsRPCManifest, {
    ...
    var menu = Menu.buildFromTemplate([
      {
        ...
        click: () => togglePinned(win, tab),
      },
      { label: 'Pop Out Tab', click: () => popOutTab(tab) },
      {
        label: 'Duplicate Tab',
        click: () => create(win, tab.url, { adjacentActive: true }),
      },
      {
        label: tab.isAudioMuted ? 'Unmute Tab' : 'Mute Tab',
        click: () => tab.toggleMuted(),
      },
      ...
      { label: 'Close Other Tabs', click: () => removeAllExcept(win, tab) },
      {
        ...
        click: () => removeAllToRightOf(win, tab),
      },
      { type: 'separator' },
  ...
  async showLocationBarContextMenu(index) {
    ...
    var menu = Menu.buildFromTemplate([
      ...
      {
        label: `Paste and ${clipInfo.isProbablyUrl ? 'Go' : 'Search'}`,
        click: onPasteAndGo,
      },
    ]);
