...
export function setup(rpc) {
  ...
  function createScopedAPI(url) {
    ...
    return {
      ...
      async getInfo(opts = {}) { return fsRPC.getInfo(url, opts) },

      async entry(path, opts = {}) { return fsRPC.entry(joinPath(url, path), opts) },
      async stat(path, opts = {}) { return createStat(await fsRPC.stat(joinPath(url, path), opts)) },
      async get(path, opts = {}) { return fsRPC.get(joinPath(url, path), opts) },
      async readFile(path, opts = {}) { return fsRPC.readFile(joinPath(url, path), opts) },
      async list(path = '/', opts = {}) { return fsRPC.list(joinPath(url, path), opts) },
      async readdir(path = '/', opts = {}) {
        const names = await fsRPC.readdir(joinPath(url, path), opts)
        if (opts.includeStats) names.forEach((n) => { n.stat = createStat(n.stat) })
        return names
      },
      async query(path = '/', opts = {}) { return fsRPC.query(joinPath(url, path), opts) },
      async diff(other, opts = {}) { return fsRPC.diff(url, other, opts) },

      async put(path, data, opts = {}) { return fsRPC.put(joinPath(url, path), data, opts) },
      async writeFile(path, data, opts = {}) { return fsRPC.writeFile(joinPath(url, path), data, opts) },
      async del(path, opts = {}) { return fsRPC.del(joinPath(url, path), opts) },
      async unlink(path, opts = {}) { return fsRPC.unlink(joinPath(url, path), opts) },
      async mkdir(path, opts = {}) { return fsRPC.mkdir(joinPath(url, path), opts) },
      async rmdir(path, opts = {}) { return fsRPC.rmdir(joinPath(url, path), opts) },
      async copy(src, dst, opts = {}) { return fsRPC.copy(joinPath(url, src), joinPath(url, dst), opts) },
      async rename(src, dst, opts = {}) { return fsRPC.rename(joinPath(url, src), joinPath(url, dst), opts) },
      async updateMetadata(path, metadata, opts = {}) { return fsRPC.updateMetadata(joinPath(url, path), metadata, opts) },
      async deleteMetadata(path, keys, opts = {}) { return fsRPC.deleteMetadata(joinPath(url, path), keys, opts) },
      async mount(path, key, opts = {}) { return fsRPC.mount(joinPath(url, path), key, opts) },
      async unmount(path, opts = {}) { return fsRPC.unmount(joinPath(url, path), opts) },
      async symlink(target, linkname, opts = {}) { return fsRPC.symlink(joinPath(url, linkname), target, opts) },
      ...
      async configure(info, opts = {}) { return fsRPC.configure(url, info, opts) },
      async forkDrive(opts = {}) { return createScopedAPI(await fsRPC.forkDrive(url, opts)) },

      // Collaborative-drive writer management (scoped to this drive)
      async createInvite(opts = {}) { return fsRPC.createInvite(url, opts) },
      async listRequests() { return fsRPC.listRequests(url) },
      ...
      async approveRequest(writerKey, opts = {}) { return fsRPC.approveRequest(url, writerKey, opts) },
      async denyRequest(writerKey) { return fsRPC.denyRequest(url, writerKey) },
      async removeWriter(writerKey) { return fsRPC.removeWriter(url, writerKey) },
      async listWriters() { return fsRPC.listWriters(url) },
    }
  }
  ...
  api.createCollaborativeDrive = async (opts = {}) => createScopedAPI(await fsRPC.createCollaborativeDrive(opts))
  api.forkDrive = async (url, opts = {}) => createScopedAPI(await fsRPC.forkDrive(massageUrl(url), opts))
  ...
  api.approveRequest = async (url, writerKey, opts = {}) => fsRPC.approveRequest(massageUrl(url), writerKey, opts)
