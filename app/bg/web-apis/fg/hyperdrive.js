...
export function setup(rpc) {
  // create the rpc apis
  const hyperdriveRPC = rpc.importAPI('hyperdrive', hyperdriveManifest, {
    timeout: false,
    errors,
  });
  ...
  function createScopedAPI(url) {
    ...
    url =
      'hyper://' +
      urlParsed.hostname +
      (urlParsed.version ? `+${urlParsed.version}` : '') +
      '/';
    ...
    return {
      ...
      async diff(prefix, other, opts = {}) {
        ...
        other =
          other && typeof other === 'object' && other.version
            ? other.version
            : other;
        var res = await hyperdriveRPC.diff(
          joinPath(url, prefix),
          other,
          prefix,
          opts
      ...
      async updateMetadata(path, metadata, opts = {}) {
        return hyperdriveRPC.updateMetadata(
          joinPath(url, path),
          metadata,
          opts
        );
      ...
      async readdir(path = '/', opts = {}) {
        ...
        if (opts.includeStats) {
          names.forEach((name) => {
            name.stat = createStat(name.stat);
          });
        }
        return names;
      },
  ...
  var api = {
    ...
    forkDrive(url, opts = {}) {
      ...
      const urlParsed = parseDriveUrl(url);
      return hyperdriveRPC
        .forkDrive(urlParsed.hostname, opts)
        .then((newUrl) => createScopedAPI(newUrl));
    },
