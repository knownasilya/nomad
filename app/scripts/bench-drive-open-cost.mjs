...
function stats(xs) {
  ...
  return { p50: p(0.5), p95: p(0.95), max: s[s.length - 1], mean: xs.reduce((a, b) => a + b, 0) / xs.length }
}
const f = (n) => n.toFixed(1)

// --- Autobase view + reducer (same shape as the ADR-0010 spike) ---------------
const open = (store) => new Hyperbee(store.get({ name: 'db' }), { keyEncoding: 'utf-8', valueEncoding: 'json' })
...
const drivers = {
  ...
  autobase: {
    async create(store) {
      const base = new Autobase(store.namespace(randomBytes(32)), null, { open, apply, valueEncoding: 'json', ackInterval: 1000 })
      await base.ready()
    ...
    async openRead(store, key) {
      const base = new Autobase(store, Buffer.from(key, 'hex'), { open, apply, valueEncoding: 'json', ackInterval: 1000 })
      await base.ready()
      await base.update()
...
async function runLongOplogSuite(opsPerDrive, count) {
  ...
  return { type: `autobase-${opsPerDrive}op`, create: stats(createMs), open: stats(openMs), correct, n: keys.length, opsPerDrive }
}
...
async function runSuite(type) {
  ...
  store = new Corestore(dir)
  await store.ready()
  const baseRss = rssMB(); const baseHeap = heapMB()
  const openMs = []
  const handles = []
  let correct = 0
  for (const key of keys) {
  ...
  return { type, create: stats(createMs), open: stats(openMs), memRss, memHeap, correct, n: keys.length }
}

function row(label, hd, ab) {
  const ratio = ab / hd
  console.log(`  ${label.padEnd(22)} ${f(hd).padStart(9)} ${f(ab).padStart(9)}   ${ratio.toFixed(1)}x`)
}
...
async function main() {
  ...
  for (const r of [hd, ab]) {
    if (r.correct !== r.n) console.log(`  ⚠️  ${r.type}: only ${r.correct}/${r.n} cold-opens read /index.json back correctly — timings suspect`)
  }
  ...
  console.log('\n  Interpretation:')
  console.log(`   • Autobase cold-open is ~${openRatio.toFixed(1)}x Hyperdrive (${f(ab.open.p50)}ms vs ${f(hd.open.p50)}ms p50).`)
  console.log(`   • Opening ${N} autobase drives at startup ≈ ${f(ab.open.mean * N)}ms serial (${f(ab.open.mean * N - hd.open.mean * N)}ms more than Hyperdrive).`)
  console.log(`   • Memory/drive ≈ ${f(ab.memRss / N)}MB autobase vs ${f(hd.memRss / N)}MB hyperdrive (~${memRatio.toFixed(1)}x).`)
  console.log('\n  Rule of thumb: if autobase cold-open p95 stays well under ~50ms and RSS/drive under a few MB,')
  console.log('  autobase-only is viable at browser scale (dozens of drives). If open scales poorly or memory')
  console.log('  balloons, prefer ADR-0010 option 2 (unified API, Hyperdrive kept for single-writer drives),')
  ...
  if (long.correct !== long.n) {
    console.log(`  ⚠️  only ${long.correct}/${long.n} long-oplog cold-opens read /index.json back — timings suspect`)
  }
  console.log(`  ${''.padEnd(22)} ${'1-op'.padStart(9)} ${(LONG_OPS + '-op').padStart(9)}   growth`)
  console.log('  ' + '-'.repeat(52))
  ...
  console.log(`  ${'create/drive (ms)'.padEnd(22)} ${''.padStart(9)} ${f(long.create.p50).padStart(9)}`)

  const openGrowth = long.open.p50 / ab.open.p50
  console.log('\n  Interpretation:')
  console.log(`   • A ${LONG_OPS}-op drive cold-opens in ${f(long.open.p50)}ms p50 / ${f(long.open.p95)}ms p95`)
  console.log(`     vs ${f(ab.open.p50)}ms / ${f(ab.open.p95)}ms for a 1-op drive (~${openGrowth.toFixed(1)}× p50).`)
  if (long.open.p95 < 50 && openGrowth < 3) {
    console.log('   • FLAT ENOUGH: open does NOT scale with edit count — Autobase checkpoints the view')
    console.log('     and replays only post-checkpoint ops. The Q2 caveat is CLOSED: long-history drives open fine.')
  } else {
    console.log('   • ⚠️ open GROWS with oplog length — Autobase is replaying a long tail on open. Investigate')
    console.log('     checkpoint cadence (ackInterval / fastForward) before committing to autobase-only for edit-heavy drives.')
  }
}

main()
  .then(() => { fs.rmSync(TMP, { recursive: true, force: true }); process.exit(0) })
  .catch((e) => { console.error('\nBENCH CRASHED:', e.stack || e); fs.rmSync(TMP, { recursive: true, force: true }); process.exit(1) })
