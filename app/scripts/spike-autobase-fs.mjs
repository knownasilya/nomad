...
async function apply(nodes, view, host) {
  for (const { value } of nodes) {
    if (!value) continue
    if (value.addWriter) { await host.addWriter(Buffer.from(value.addWriter, 'hex'), { indexer: true }); continue }
    if (value.op === 'put') { await view.put(value.path, value.record); continue }
    if (value.op === 'del') { await view.del(value.path); continue }
  }
}
...
async function putFile(dev, buf, meta = {}) {
  ...
  return { metadata: { mtime: meta.mtime ?? 1, ctime: meta.ctime ?? 1, size: buf.length }, blob: { core: dev.blobs.key.toString('hex'), id } }
}
...
async function readRecord(dev, p) { const n = await dev.base.view.get(p); return n && n.value }
async function dumpView(dev) {
  const out = []
  for await (const e of dev.base.view.createReadStream()) out.push(`${e.key}=${JSON.stringify(e.value)}`)
...
async function main() {
  ...
  await A.base.append({ op: 'put', path: '/index.json', record: { metadata: { mtime: 1, ctime: 1 }, blob: null } })
  await A.base.update()
  const B = await makeDevice('B', A.base.key)              // bootstrap on A's key
  let disconnect = connect(A, B)
  await waitFor(async () => { await B.base.update(); return !!(await readRecord(B, '/index.json')) }, 15000)
  ...
  await A.base.update()
  const bWritable = await waitFor(async () => { await A.base.update(); await B.base.update(); return B.base.writable }, 20000)
  ...
  await A.base.update()
  const gotPtr = await waitFor(async () => { await B.base.update(); return !!(await readRecord(B, '/big.bin')) }, 15000)
  check('B sees the /big.bin pointer in the view', gotPtr)
  const recB = await readRecord(B, '/big.bin')
  check('pointer carries { core, id } (no inline bytes)', !!(recB?.blob?.core && recB?.blob?.id && recB.blob.id.byteLength === big.length))
  const full = await withTimeout(getFile(B, recB), 60000, 'online-full').catch((e) => e)
  check('B reads A\'s full blob, bytes match', Buffer.isBuffer(full) && sha(full) === bigHash, Buffer.isBuffer(full) ? '' : String(full.message || full))
  const mid = await withTimeout(getFile(B, recB, { start: 1_000_000, length: 65536 }), 30000, 'online-range').catch((e) => e)
  check('B range-reads [1MB, +64KB), bytes match', Buffer.isBuffer(mid) && Buffer.compare(mid, big.subarray(1_000_000, 1_000_000 + 65536)) === 0)
  ...
  await sleep(300)
  const offline = await withTimeout(getFile(B, recB, { start: 0, length: 131072 }), 8000, 'offline-cached').catch((e) => e)
  check('B serves cached blob with A offline', Buffer.isBuffer(offline) && Buffer.compare(offline, big.subarray(0, 131072)) === 0,
  ...
  await A.base.update()
  const unreadable = await withTimeout(getFile(B, ptr2, { start: 0, length: 16 }), 4000, 'unavailable')
  ...
  await waitFor(async () => { await A.base.update(); await B.base.update(); return !!(await readRecord(B, '/unseen.bin')) }, 15000)
  ...
  const converged = await waitFor(async () => {
    await A.base.update(); await B.base.update()
    const da = await dumpView(A), db = await dumpView(B)
  ...
  check('post-reorg pointers still resolve both directions',
    Buffer.isBuffer(crossB) && crossB.length === 20000 && Buffer.isBuffer(crossA) && crossA.length === 20000)
...
main()
  .then(() => {
    ...
    console.log('Verdict:', failed.length ? 'option-2 (hybrid backend) — Autobase-only blob model not proven' : 'Autobase-only model VALIDATED — proceed with ADR-0010 option (b)')
