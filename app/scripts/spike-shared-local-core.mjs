...
async function main () {
  ...
  await vault.update()
  console.log('  Vault ready + writable:', vault.writable, '| local key', b4a.toString(vault.local.key, 'hex').slice(0, 8))
  ...
  const s1 = store.replicate(true), s2 = other.replicate(false)
  ...
  try {
    ...
    console.log('  open — the shared exclusive `local` core blocks. Paired-drive writable open needs a')
    console.log('  different writer model (per-drive device key, or a Vault-mediated writer).')
    await cleanup(store, other, vault, space); process.exit(1)
  }
  ...
  const writable = await (async () => {
    ...
    while (!secondBase.writable && Date.now() < deadline) { await new Promise(r => setTimeout(r, 200)); await secondBase.update() }
  ...
  if (writable) {
    ...
    try {
      await withTimeout(secondBase.append({ op: 'put', path: '/from-device', data: { ok: true } }), 8000, 'second-append')
      await secondBase.update()
      await withTimeout(vault.append({ op: 'put', path: '/vault-2', data: { ok: true } }), 8000, 'vault-append')
      await vault.update()
...
async function cleanup (...xs) {
  for (const x of xs) { try { await x.close?.() } catch {} }
