...
const litElementPath = path.join(
  __dirname,
  '..',
  'vendor',
  'lit-element',
...
function handleCSSFile(cssPath) {
  ...
  fs.writeFileSync(
    cssJsPath,
    `import {css} from '${path.relative(path.dirname(cssPath), litElementPath)}'
${imports}
const cssStr = css\`
${newCss}
\`
...
function extractAndReplaceImports(css) {
  ...
  var importsStr = imports
    .map((i) => `import ${i.varname} from '${i.path}'`)
    .join('\n');
