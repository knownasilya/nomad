...
var emojiDataStr = fs.readFileSync(
  require('path').join(__dirname, 'emoji-data.txt'),
  'utf8'
);
...
for (let groupStr of emojiDataStr.split('# group: ').slice(1)) {
  ...
  while ((match = re.exec(groupStr))) {
    let emoji = match[1]
      .trim()
      .split(' ')
      .map((v) => String.fromCodePoint(parseInt(v, 16)))
      .join(''); // parse out emoji
...
fs.writeFileSync(
  require('path').join(__dirname, 'emoji-list.js'),
  `
