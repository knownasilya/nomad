...
export async function diff(
  ...
  async function walk(basePath, targetPath) {
    ...
    baseNames.forEach((name) => {
      if (targetNames.indexOf(name) === -1) {
        ps.push(
          addRecursive(joinPath(basePath, name), joinPath(targetPath, name))
        );
      } else {
    ...
    targetNames.forEach((name) => {
      if (baseNames.indexOf(name) === -1) {
        ps.push(
          delRecursive(joinPath(basePath, name), joinPath(targetPath, name))
        );
      } else {
        // already handled
      }
    ...
    if (baseStat.mount && targetStat.mount) {
      if (baseStat.mount.key !== targetStat.mount.key) {
        changes.push({
          ...
          basePath,
          targetPath,
          baseMountKey: baseStat.mount.key,
          targetMountKey: targetStat.mount.key,
        });
      }
      return;
    }
  ...
  async function diffFile(basePath, baseStat, targetPath, targetStat) {
    ...
    if (!isEq && opts.compareContent) {
      ...
      if (compareContentCache) {
        ...
        if (
          cacheEntry &&
          cacheEntry.baseMtime === +baseStat.mtime &&
          cacheEntry.targetMtime === +targetStat.mtime
        ) {
  ...
  async function addRecursive(basePath, targetPath, isFirstRecursion = false) {
    ...
    if (
      !isFirstRecursion /* when first called from diff(), dont check for a cycle again */
    ) {
