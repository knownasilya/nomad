...
export class CompareApp extends LitElement {
  ...
  async load() {
    if (!this.otherDrives) {
      (async () => {
        ...
        this.otherDrives = this.otherDrives.filter((drive) => {
          if (drive.forkOf) {
            let parent = this.otherDrives.find(
              (d) => d.key === drive.forkOf.key
            );
            if (parent) {
    ...
    if (this.baseInfo && this.targetInfo) {
      ...
      this.diff = await this.attempt('Comparing files', () =>
        compare.diff(
          this.baseDrive,
          this.basePath,
          this.targetDrive,
          this.targetPath,
          { compareContent: true, shallow: false, filter }
        )
      );
    } else {
      this.diff = [];
  ...
  async doMerge(diff) {
    try {
      ...
      console.error(e);
      toast.create(
        e.message || 'There was an issue writing the files',
        'error'
      );
    }
    this.load();
  ...
  render() {
    ...
    const diffSortFn = (a, b) =>
      a.change.localeCompare(b.change) ||
      (a.path || '').localeCompare(b.path || '');
    var numChecked = `${this.checkedItems?.length} ${pluralize(
      this.checkedItems?.length,
      'Change'
    )}`;
    var counts = {
      additions: this.checkedItems?.filter?.((d) => d.change === 'add')?.length,
      modifications: this.checkedItems?.filter?.((d) => d.change === 'mod')
        ?.length,
        ...
        <div class="toolbar">
          <div class="title">
            ...
            <div class="btn-group">
              ...
              ${this.baseInfo && this.baseForks && this.baseForks.length > 1
                ? html`
      ...
      <div class="layout">
        <nav @click=${this.onClickOutside}>
          <div class="nav-header">
            <label
              ><input type="checkbox" @change=${this.onToggleAllChecked} />
              Select / Deselect All</label
            >
          </div>
          ${!this.diff
            ? html`<div style="padding: 5px">Comparing...</div>`
            : ''}
          ${this.diff && !this.diff.length
            ? html` <div class="empty">No differences found.</div> `
            : ''}
          ${repeat((this.diff || []).slice().sort(diffSortFn), (diff) => {
            let heading =
              lastDiffType !== diff.change
                ? html`
                    <h4>
                      ${{ add: 'Add', del: 'Delete', mod: 'Modification' }[
                        diff.change
                      ]}
                    </h4>
                  `
                ...
                ${this.taskState.error
                  ...
                  : html`
                      <div class="summary">
                        <h2>Loading...</h2>
                        <div class="task">
                          <span class="spinner"></span> ${this.taskState.task}
                        </div>
                      </div>
                    `}
              `
            ...
            : html`
                <div class="summary">
                  ${this.diff?.length > 0
                    ? html`
                        <h2>
                          Merging ${this.checkedItems?.length}
                          ${pluralize(this.checkedItems?.length, 'Change')}
                        </h2>
                      `
                    : html`
                        <h2>
                          <span class="fas fa-check"></span> No Differences
                          Found
                        </h2>
                      `}
                  <div>
                    <span class="revision-indicator add"></span>
                    ${counts.additions}
                    ${pluralize(counts.additions, 'addition')}
                  </div>
                  <div>
                    <span class="revision-indicator mod"></span>
                    ${counts.modifications}
                    ${pluralize(counts.modifications, 'modification')}
                  </div>
                  <div>
                    <span class="revision-indicator del"></span>
                    ${counts.deletions}
                    ${pluralize(counts.deletions, 'deletion')}
                  </div>
                  <p>
                    Merging
                    ${this.baseInfo
                      ? html`
                          <a href="${this.baseInfo.url}" target="_blank">
                            ...
                            ${this.baseFork?.forkOf?.label
                              ? html`<span class="fork-label"
                                  >${this.baseFork?.forkOf?.label}</span
                                >`
                              : ''}
                          </a>
                        `
                    ...
                    ${this.targetInfo
                      ? html`
                          <a href="${this.targetInfo.url}" target="_blank">
                            ...
                            ${this.targetFork?.forkOf?.label
                              ? html`<span class="fork-label"
                                  >${this.targetFork?.forkOf?.label}</span
                                >`
                              : ''}
                          </a>
                        `
                  ...
                  ${this.targetInfo?.writable
                    ? html`
                        <p>
                          <button
                            class="primary"
                            ?disabled=${this.checkedItems?.length === 0}
                            @click=${this.onClickBulkMerge}
                          >
                            Merge ${numChecked}
                          </button>
                        </p>
                    ...
                    : html`
                        ...
                        <div class="share-link-instructions">
                          <div>
                            Send this link to the author and ask them to merge:
                          </div>
  ...
  updated() {
    ...
    var checked =
      this.diff?.length > 0 && this.checkedItems?.length === this.diff?.length;
...
class CompareDiffItem extends LitElement {
  ...
  render() {
    ...
    return html`
      <div
        class="item ${this.diff.change} ${this.selected ? 'selected' : ''}"
        @click=${this.onSelect}
      >
        ...
        <div class="path">
          ${relativePath(this.targetPath, this.diff.targetPath)}
        </div>
      </div>
    `;
  }
...
class CompareDiffItemContent extends LitElement {
    ...
    return html`
      <div class="info">
        <span class="path">
          ...
          ${this.diff.change === 'mod' ? 'Modify' : ''} ${this.diff.targetPath}
        </span>
        ${this.canMerge
          ? html` <button @click=${this.onClickMerge}>Merge</button> `
          : html`
              <button
                disabled
                data-tooltip="Can't merge into a site you don't own"
              >
                Merge
              </button>
            `}
        ${['del', 'mod'].includes(this.diff.change)
          ? html`
              <a
                href="${this.targetOrigin}${this.diff?.targetPath}"
                target="_blank"
                ><span class="fas fa-fw fa-external-link-alt"></span> View
                current</a
              >
            `
          : ''}
        ${['add', 'mod'].includes(this.diff.change)
          ? html`
              <a href="${this.baseOrigin}${this.diff?.basePath}" target="_blank"
                ><span class="fas fa-fw fa-external-link-alt"></span> View new
                file</a
              >
            `
          : ''}
  ...
  renderDiff() {
    if (
      this.diff.type === 'mount' ||
      this.diff.type === 'dir' ||
      isFilenameBinary(this.diff?.basePath || this.diff?.targetPath)
    ) {
      if (this.diff.change === 'mod') {
        return html`
          <div class="container split">
            <div>
              <div class="action">From</div>
              <div class="wrap">${this.renderLeftColumn()}</div>
            </div>
            <div>
              <div class="action">To</div>
              <div class="wrap">${this.renderRightColumn()}</div>
            </div>
          </div>
        `;
      } else if (this.diff.change === 'add') {
        return html`<div class="container">
          <div><div class="wrap">${this.renderRightColumn()}</div></div>
        </div>`;
      } else if (this.diff.change === 'del') {
        return html`<div class="container">
          <div><div class="wrap">${this.renderLeftColumn()}</div></div>
        </div>`;
      }
    } else if (this.diff) {
  ...
  renderFileContent(drive, path, mountKey) {
    if (this.diff.type === 'mount') {
      return html`
        ...
        Mount to
        <a href="hyper://${mountKey}" target="_blank"
          >${toNiceDomain(mountKey)}</a
        >
      `;
    }
    if (this.diff.type === 'dir')
      return html`<span class="fas fa-fw fa-folder"></span> Directory`;
