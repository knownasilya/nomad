...
class DriveView extends LitElement {
  static get styles() {
    return css`
      :host {
        --sans-serif: -apple-system, BlinkMacSystemFont, system-ui, 'Segoe UI',
  ...
  async load() {
    ...
    var entries = await this.drive.readdir(location.pathname, {
      includeStats: true,
    });
    entries.sort((a, b) => {
      ...
      <main>
        ...
        <div class="entries">
          ${location.pathname !== '/'
            ? html`
                <div>
                  <a href=".." title="Go up a directory"
                    ><span class="fa-fw fas fa-level-up-alt"></span> ..</a
                  >
                </div>
              `
            : ''}
        ...
        ${this.info.writable
          ? html`
              <div class="prompt">
                Create an
                <a @click=${(e) => this.onCreateIndex(e, 'md')} href="#"
                  >index.md</a
                >
                or
                <a @click=${(e) => this.onCreateIndex(e, 'html')} href="#"
                  >index.html</a
                >
                in this directory.
              </div>
  ...
  renderEntry(entry) {
    ...
    return html`
      <div>
        ...
        ${entry.stat.size
          ? html` <span class="size">${bytes(entry.stat.size)}</span> `
          : ''}
      </div>
    `;
  }
