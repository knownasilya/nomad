...
class FilesExplorer extends LitElement {
  ...
  render() {
    ...
    const icon = (item) => {
      ...
      if (item.stat.isDirectory())
        return html`<span class="fa-fw fas fa-folder"></span>`;
    ...
    return html`
        ...
        <a>
          ...
          ${this.currentFolder.mount
            ? html`(<code>${this.currentFolder.mount.key.slice(
                0,
                4
              )}..${this.currentFolder.mount.key.slice(-2)}</code>)`
            : ''}
        </a>
      ...
      <div class="listing" @contextmenu=${this.onContextmenuListing}>
        ${this.folderPath !== '/'
          ? html`
              <div class="item" @click=${this.onClickUpdog}>
                <span class="icon"
                  ><span class="fa-fw fas fa-level-up-alt"></span
                ></span>
        ...
        ${repeat(
          ...
          (item) => html`
            <div
              class=${classMap({
                item: true,
                selected: item.path === this.openFilePath,
              })}
        ...
        ${!this.readOnly
          ? html`
              <div
                class=${classMap({ item: true, 'new-file': true })}
  ...
  onContextmenuItem(e, item) {
    ...
    e.stopPropagation();
    emit(this, 'show-menu', {
      detail: { x: e.clientX, y: e.clientY, folderPath: this.folderPath, item },
    });
  }

  onClickUpdog(e) {
    var upPath = this.folderPath
      .split('/')
      .filter(Boolean)
      .slice(0, -1)
      .join('/');
