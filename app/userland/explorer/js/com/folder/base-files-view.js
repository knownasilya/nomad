...
export class BaseFilesView extends LitElement {
  ...
  endDragDropMode() {
    if (this.dragDropModeActive) {
      ...
      try {
        this.shadowRoot.querySelector('.drag-ghost').remove();
      } catch (e) {}
    }
    Array.from(this.shadowRoot.querySelectorAll('.drag-hover'), (el) =>
  ...
  render() {
    ...
    return html`
      ...
      >
        ...
        ${isEmpty ? html` <div class="empty">This folder is empty</div> ` : ''}
        ${inlineMdItem
          ? html`
              <h4>Index.md</h4>
              <div class="readme">
  ...
  onClickItem(e, item) {
    ...
    if (e.metaKey || e.ctrlKey) {
      ...
      let selector = {
        start: getElXY(this.lastClickedItemEl),
        current: getElXY(e.currentTarget),
      };
      let els = findElsInSelector(
        selector,
        this.shadowRoot.querySelectorAll('.item')
      );
      let items = els.map((el) =>
        this.items.find((i) => i.url === el.dataset.url)
      );
  ...
  onDropItem(e, item) {
    ...
    var targetPath =
      item && item.stat.isDirectory() ? item.path : loc.getPath();
    handleDragDrop(
      e.currentTarget,
      e.clientX,
      e.clientY,
    ...
    if (this.dragSelector && !this.dragDropModeActive) {
      if (!e.buttons) {
        ...
        if (!this.dragSelector.isActive) {
          ...
          if (
            Math.abs(this.dragSelector.current.x - this.dragSelector.start.x) >
              50 ||
            Math.abs(this.dragSelector.current.y - this.dragSelector.start.y) >
              50
          ) {
        ...
        if (this.dragSelector.isActive) {
          ...
          var newSelectedEls = findElsInSelector(
            this.dragSelector,
            this.shadowRoot.querySelectorAll('.item')
          );
          var newSelection = newSelectedEls.map((el) =>
            this.items.find((i) => i.url === el.dataset.url)
          );
          if (diffMode) {
  ...
  onDragleaveContainer(e) {
    ...
    if (itemEl && itemEl !== this.dragLastEntered) {
      if (this.dragLastEntered)
        this.dragLastEntered.classList.add('drag-hover');
...
function positionDragSelector(dragSelector) {
  function min(k) {
    return Math.min(dragSelector.start[k], dragSelector.current[k]);
  }
  function max(k) {
    return Math.max(dragSelector.start[k], dragSelector.current[k]);
  }
