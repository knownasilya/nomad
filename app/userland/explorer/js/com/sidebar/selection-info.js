...
export class SelectionInfo extends LitElement {
  ...
  render() {
    if (this.selection.length > 1) {
      return html`
        <section>
          <strong>${this.selection.length} items selected</strong>
        </section>
      `;
    }
    ...
    return html`
      ...
      <section>
        ...
        ${this.renderSize()}
        ${sel.mount
          ? html` <drive-info .driveInfo=${sel.mount}></drive-info> `
          : ''}
        ${!this.noPreview && sel.stat.isFile()
          ? html`
  ...
  renderSize() {
    ...
    return html`<p class="facts">
      <span><span class="fas fa-fw fa-save"></span> ${bytes(sz)}</span>
    </p>`;
  }

  renderMetadata() {
    if (this.selection[0].stat.isDirectory()) return;
    var isWritable = this.mountInfo
      ? this.mountInfo.writable
      : this.driveInfo.writable;
    ...
    return html`
      ...
      ${this.metadataHasChanged
        ? html`
            <button class="primary" @click=${this.onClickSaveMetadata}>
  ...
  async onClickSaveMetadata(e) {
    ...
    emit(this, 'update-file-metadata', {
      detail: { path: this.selection[0].path, newMetadata, deletedKeys },
    });
    ...
    this.querySelector(
      '.metadata .entry:last-of-type input[name="key"]'
    ).value = '';
    this.querySelector(
      '.metadata .entry:last-of-type input[name="value"]'
    ).value = '';
  }
}
