        ...
        ${this.mergeMode
          ? html`
              <div class="label" style="margin-top: 5px">
                <span class="fas fa-fw fa-compress-arrows-alt"></span> Merging
                folders by ${this.mergeMode}
              </div>
            `
          : ''}
        <pre style="margin-top: 5px">
${JSON.stringify(this.viewfileObj.query, null, 2)}</pre
        >
      </section>
    `;
  }
  ...
  renderDrive() {
    ...
    return html`<span
      ><small>Drive:</small>
      <a href=${drive.url} title=${drive.title}>${drive.title}</a></span
    >`;
  }
