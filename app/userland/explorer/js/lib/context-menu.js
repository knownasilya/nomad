...
export function constructItems(app) {
  ...
  if (app.selection.length === 1 || app.pathInfo.isFile()) {
    ...
    let writable = app.selection.reduce(
      (acc, v) => acc && v.drive.writable,
      true
    );
    items.push({
    ...
    items.push({
      icon: html`
        <i class="fa-stack" style="font-size: 6px">
          ...
          <span
            class="fas fa-fw fa-share fa-stack-1x"
            style="margin-left: -10px; margin-top: -5px; font-size: 7px"
          ></span>
        </i>
      `,
      label: 'Copy Address',
    ...
    if (!app.isViewingQuery) {
      ...
      if (!sel.stat.isFile()) {
        items.push({
          icon: html`<i
            style="padding-left: 2px; font-size: 16px; box-sizing: border-box"
            >◨</i
          >`,
          label: 'Diff / merge',
