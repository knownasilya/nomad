    ...
    if (i.stat.mount && i.stat.mount.key) {
      switch (i.mount.type) {
        case 'walled.garden/person':
          ...
          break;
        case 'website':
          add('websites', 'Websites', i);
          break;
        case 'application':
          add('applications', 'Applications', i);
          break;
        case 'webterm.sh/cmd-pkg':
          add('commands', 'Webterm Commands', i);
          break;
        default:
          add('drives', 'Drives', i);
      }
...
async function doCopyOrMove({ sourceItem, targetFolder }, op) {
  ...
  var sourceDrive = beaker.fs.drive(sourceItemParsed.hostname);
  let targetFolderParsed = new URL(targetFolder);
  var targetDrive = beaker.fs.drive(targetFolderParsed.hostname);
  ...
  if (targetSt) {
    if (
      targetSt.isFile() &&
      !confirm(`${name} already exists in the target folder. Overwrite?`)
    ) {
      throw new Error('Canceled');
    } else if (targetSt.isDirectory()) {
      alert(
        `A folder named "${name}" already exists in the target folder and cannot be overwritten.`
      );
      throw new Error('Canceled');
    }
  }
...
export async function doCopy(params) {
  return doCopyOrMove(
    params,
    (sourceDrive, sourcePath, targetDrive, targetPath) =>
      sourceDrive.copy(sourcePath, joinPath(targetDrive.url, targetPath))
  );
}
...
export async function doMove(params) {
  return doCopyOrMove(
    params,
    (sourceDrive, sourcePath, targetDrive, targetPath) =>
      sourceDrive.rename(sourcePath, joinPath(targetDrive.url, targetPath))
  );
}
    ...
    return new Promise((resolve, reject) => {
      ...
      dirReader.readEntries(async (entries) => {
        try {
          ...
          var targetSt = await targetDrive
            .stat(targetPath)
            .catch((e) => undefined);
          if (targetSt) {
            if (
