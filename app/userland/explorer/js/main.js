...
import {
  getDriveTitle,
  getGlobalSavedConfig,
  setGlobalSavedConfig,
  getVFCfg,
} from './lib/config.js';
...
export class ExplorerApp extends LitElement {
  ...
  async load() {
    if (this.attachedPane) {
      if (!loc.getUrl() || !loc.getUrl().startsWith('hyper://')) {
        ...
        return window.close();
        this.errorState = {
          task: 'Attempting to load',
          error: new Error('Failed to load this address: not a hyperdrive'),
        };
        return;
      }
    } else if (!loc.getUrl()) {
    ...
    try {
      ...
      this.mountTitle = this.mountInfo
        ? getDriveTitle(this.mountInfo)
        : undefined;
      document.title = this.filename
        ? `${this.driveTitle} / ${this.filename}`
        : this.driveTitle;

      this.pathInfo = await this.attempt(
    ...
    if (this.pathInfo.isDirectory()) {
      ...
      this.renderMode = getGlobalSavedConfig(
        'render-mode',
        getVFCfg(this.viewfileObj, 'renderMode', ['grid', 'list']) || 'grid'
      );
      this.inlineMode = Boolean(
        getGlobalSavedConfig(
          'inline-mode',
          getVFCfg(this.viewfileObj, 'inline', [true, false]) || false
        )
      );
    ...
    if (this.selection && this.selection.length) {
      this.selection = this.selection
        .map((item) => this.items.find((item2) => item2.path === item.path))
        .filter(Boolean);
    }
  ...
  async readViewfile(drive) {
    ...
    if (getVFCfg(this.viewfileObj, 'merge', ['mtime', undefined])) {
      ...
      for (let item of items) {
        if (item.name in map) {
          map[item.name] =
            map[item.name].stat.mtime > item.stat.mtime ? map[item.name] : item;
        } else {
          map[item.name] = item;
        }
      }
  ...
  get renderModes() {
    if (this.pathInfo.isDirectory()) {
      return [
        ['grid', 'th-large', 'Files Grid'],
        ['list', 'th-list', 'Files List'],
      ];
    } else {
      if (loc.getPath().endsWith('.md') || loc.getPath().endsWith('.goto')) {
        return [
          ['default', 'file', 'Rendered'],
          ['raw', 'code', 'Source'],
        ];
      }
        ...
        ${this.loadingState === LOADING_STATES.INITIAL
          ...
          : html`
              <main @contextmenu=${this.onContextmenuLayout}>
                ${this.renderHeader()}
                ${this.loadingState === LOADING_STATES.CONTENT
                  ? html` <div class="loading-notice">Loading...</div> `
                  : ''}
  ...
  renderInitialLoading() {
    ...
    return html`
      <div class="loading-view">
        <div><span class="spinner"></span> Searching the network...</div>
        ${this.errorState && this.errorState.error.name === 'TimeoutError'
          ? html`
              <div
                style="margin-top: 10px; margin-left: 27px; font-size: 12px; opacity: 0.75;"
              >
                We're having some trouble
                ${this.errorState.task.toLowerCase()}.<br />
      ...
      <div class="header">
        ...
        <button class="primary labeled-btn" @click=${this.onClickActions}>
          ${this.selection.length
            ? `${this.selection.length} ${pluralize(
                this.selection.length,
                'item'
              )} selected`
            : this.pathInfo.isDirectory()
            ? 'Folder'
            : 'File'}
        ...
        ${this.attachedPane
          ? html`
              <button class="transparent" @click=${window.close}>
  ...
  renderView() {
    if (
      this.items.length === 0 &&
      (this.loadingState === LOADING_STATES.CONTENT || this.errorState)
    ) {
    ...
    const isViewfile =
      this.pathInfo.isFile() && loc.getPath().endsWith('.view');
    if (isViewfile) {
      return html`
        <explorer-view-query
  ...
  renderErrorState() {
    if (
      !this.errorState ||
      this.errorState.error.message.includes('TimeoutError')
    )
      return undefined;
      ...
      return html`
        <div class="error-view">
          <div class="error-title">
            <span class="fas fa-fw fa-exclamation-triangle"></span> File or
            folder not found
          </div>
    ...
    return html`
      <div class="error-view">
        <div class="error-title">
          <span class="fas fa-fw fa-exclamation-triangle"></span> Something has
          gone wrong
        </div>
        <div class="error-task">
          While ${this.errorState.task.toLowerCase()}
        </div>
        <details>
          <summary>
  ...
  async onClickActions(e) {
    ...
    await this.onShowMenu(
      { detail: { x: rect.right, y: rect.bottom, right: true } },
      true
    );
  ...
  async onClickSettings(e) {
    ...
    el.classList.add('active');
    await settingsMenu.create(this, {
  ...
  async onExport(e) {
    var urls = (this.selection.length ? this.selection : this.items).map(
      (item) => item.url
    );
  ...
  async onDelete(e) {
    ...
    try {
      if (this.selection.length) {
        if (
          !confirm(
            `Delete ${this.selection.length} ${pluralize(
              this.selection.length,
              'item'
            )}?`
          )
  ...
  onClickShare(e) {
    ...
    shareMenu.create({
      ...
      targetLabel: (this.selection[0]
        ? this.selection[0].stat
        : this.pathInfo
      ).isDirectory()
        ? 'folder'
        : 'file',
