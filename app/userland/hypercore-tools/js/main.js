...
class HypercoreToolsApp extends LitElement {
  ...
  bindCoreLogEvents(core, url, corename) {
    ...
    events.addEventListener('peer-open', (peer) =>
      log('Peer Connected:', peer.remoteAddress)
    );
              ...
              ${this.selectedDrive
                ? html`
                    <div class="drives-list">
                      ...
                      <div class="drives-list-columns">
                        <div class="list">
                          ${this.drivecores.map(
                            this.renderDriveListItemShort.bind(this)
                          )}
                        </div>
                        <div class="view">
                          ${this.currentDriveView === 'cores'
                ...
                : html`
                    <div class="drives-list">
                      ...
                      ${this.drivecores.map(
                        this.renderDriveListItemFull.bind(this)
                      )}
                    </div>
                  `}
            `
          : ''}
        ${this.currentView === 'api-calls'
          ? html`
              <div
                class="api-calls-grid ${this.selectedAPICall !== undefined
                  ? 'two'
                  : 'one'}"
              >
                <div>${this.renderAPICalls()}</div>
                ${this.selectedAPICall !== undefined
                  ? html` <div>${this.renderSelectedAPICall()}</div> `
                  : ''}
              </div>
            `
          : ''}
  ...
  renderDriveListItemShort(drive, index) {
    ...
    return html`
      <div
        class="drives-list-item ${this.selectedDrive === drive
          ? 'selected'
          : ''}"
  ...
  renderDriveListItemFull(drive, index) {
    let keyStr = toHex(drive.metadata.key);
    let totalBlocks =
      drive.metadata.totalBlocks + (drive.content.totalBlocks || 0);
    let downloadedBlocks =
      drive.metadata.downloadedBlocks + (drive.content.downloadedBlocks || 0);
    return html`
      <div
        class="drives-list-item ${this.selectedDrive === drive
          ? 'selected'
      ...
      >
        ...
        <div class="initiator">
          ${index === 0 ? 'Browser' : `Mount (${drive.path})`}
        </div>
        <div class="peers">${drive.metadata.peers}</div>
        <div class="blocks">
          ${totalBlocks} (${Math.round((downloadedBlocks / totalBlocks) * 100)}%
          downloaded)
        </div>
      </div>
    `;
  ...
  renderDriveCores(drive) {
    return html`
      <div class="drive">
        ${drive.path !== '/'
          ? html`<div class="mount-path">Mounted at ${drive.path}</div>`
          : ''}
    ...
    return html`
      <section class="core">
        ...
        <div class="stats">
          ...
          <span class="blocks-summary"><small>Blocks:</small> ${
            core.totalBlocks
          }</span>
        </div>
  ...
  renderFiles(drive) {
    if (drive.filesError) {
      return html`<section class="files error">
        <div class="label">Files</div>
        ${drive.filesError}
      </section>`;
    }
    if (!drive.files) {
      return html`<section class="files loading">
        <div class="label">Files</div>
        ...
        ${drive.files.map((file) => {
          ...
          return html`
            <div class="file ${d ? 'downloaded' : ''}">
              <span class="indicator"></span>
              <a class="path" href="${drive.url}/${file.name}" target="_blank"
                >/${file.name}</a
              >
    ...
    return html`
      <div class="api-calls">
        <table class="rows">
          ...
          <tbody>
            ${this.apiCallLog.map((row, i) => {
              return html`
                <tr
                  class="logger-row ${this.selectedAPICall === i
                    ? 'selected'
                    : ''}"
                  @click=${(e) => {
                    this.selectedAPICall = i;
                  }}
                >
  ...
  onMouseoverBlock(e) {
    ...
    this.setHoverEl(true, {
      label: `Block ${e.currentTarget.dataset.index} (${
        e.currentTarget.dataset.downloaded == '1' ? '' : 'Not '
      }Downloaded)`,
      left: rect.left,
      top: rect.bottom + 10,
    });
