...
class BlockingSettingsView extends LitElement {
    ...
    return html`
      <div class="section">
        <div class="adblock-settings-list">
          ${this.settings.adblock_lists.map((adblockList, i) => {
            return html` <div class="checkbox-item">
              ${this.settings.adblock_lists.length === 1
                ? ''
                : html`
                    <a
                      @click="${() => this.removeAdblockList(i)}"
                      data-tooltip="Remove"
                      title="Remove Adblock List"
                    >
      ...
      <form @submit=${this.onAddAdblockList}>
        <input
          type="text"
          placeholder="Name"
          id="custom-adblock-list-name"
          required
        />
        <input
          type="url"
          placeholder="URL"
          id="custom-adblock-list-url"
          required
        />
  ...
  onAddAdblockList(e) {
    ...
    this.settings.adblock_lists.push({
      name: name.value,
      url: url.value,
      selected: true,
    });
