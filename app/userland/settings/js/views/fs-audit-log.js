        ...
        ${this.stats
          ? html`
              <table>
                <tr>
                  <th>Average:</th>
                  <td>${this.stats.runtime.avg | 0}ms</td>
                </tr>
                <tr>
                  <th>Std Deviation:</th>
                  <td>${this.stats.runtime.stdDev | 0}ms</td>
                </tr>
                <tr>
                  <th>10 longest:</th>
                  <td>
                    <details>
                      <summary>
                        ${this.stats.runtime.longest10
                          .map((row) => row.runtime + 'ms')
                          .join(', ')}
                      </summary>
                      <pre>
${JSON.stringify(this.stats.runtime.longest10, null, 2)}</pre
                      >
                    </details>
                  </td>
                </tr>
            ...
            ${this.rows.map((row, i) => {
              ...
              return html`
                ${tsDiff > 5e3
                  ...
                  : ''}
                <tr
                  class="logger-row ${row.runtime > 250
                    ? 'badish'
                    : ''} ${row.runtime > 1e3 ? 'bad' : ''}"
                >
                  <td class="caller">
                    ${row.caller && !row.caller.startsWith('-')
                      ? html`
                          <a href=${row.caller} target="_blank"
                            >${toNiceDomain(row.caller || '')}</a
                          >
                        `
                      : row.caller}
                  </td>
                  <td class="target">
                    ${row.target
                      ? html`
                          <a href=${'hyper://' + row.target} target="_blank"
                            >${toNiceDomain(row.target)}</a
                          >
                        `
                      : ''}
                  </td>
                  <td class="ts">
                    <code>${timeDifference(row.ts, true)}</code>
                  </td>
