...
class LogSettingsView extends LitElement {
  ...
  renderControls() {
    const option = (curr, v, label) => {
      return html`<option value="${v}" ?selected=${curr === v}>
        ${label}
      </option>`;
    };
    return html` <div class="controls">
      <div class="standard-controls">
        <button class="btn transparent" @click=${this.onToggleControlsExpanded}>
          <i
            class="fas fa-${this.isControlsExpanded
              ? 'angle-down'
              : 'angle-up'}"
          ></i>
        </button>
        ...
        ${AVAILABLE_CATEGORIES.map((category) =>
          this.renderCategoryFilter(category)
        )}
        <span class="divider"></span>
        <span class="status">
      ...
      ${this.isControlsExpanded
        ? this.settings.customRules.map(
            (rule) => html`
              <div class="custom-filter-controls">
                <input
                  type="text"
                  value=${rule.query}
                  @change=${(e) => this.onChangeCustomRuleQuery(e, rule)}
                />
                <select
                  @change=${(e) => this.onChangeCustomRuleEffect(e, rule)}
                >
                  ${option(
                    rule.effect,
                    'exclude-nonmatches',
                ...
                <button
                  ...
                  @click=${() => this.onRemoveCustomRule(rule)}
                >
    ...
    return html`
      <label>
        <input
          type="checkbox"
          ?checked=${isChecked}
          @change=${() => this.onToggleLevelFilter(level)}
        />
    ...
    return html`
      <label>
        <input
          type="checkbox"
          ?checked=${isChecked}
          @change=${() => this.onSelectCategory(category)}
        />
        <i class="far ${isChecked ? 'fa-check-circle' : 'fa-circle'}"></i>
        ${category}
      </label>
    `;
  }
  ...
  renderRow(row, i) {
    return html`
      <tr class="logger-row ${row.highlight || ''}">
        ...
        <td class="msg">
          ${row.message} ${row.details ? renderDetails(row.details) : ''}
        </td>
