...
class NetworkView extends LitElement {
  ...
  render() {
    return html`
      ...
      ${this.error
        ...
        : this.networkStatus
        ? html`
            ${this.networkStatus.length === 0
              ? html` <p><em>No active drives</em></p> `
              : ''}
            <table>
              ${repeat(
                this.networkStatus,
                (v, i) => i,
                (item) => this.renderDriveStatus(item)
              )}
            </table>
          `
  ...
  renderDriveStatus(item) {
    ...
    var domain =
      drive && drive.ident.system
        ? 'private'
        : `${key.slice(0, 6)}..${key.slice(-2)}`;
    var title =
      drive && drive.info && drive.info.title
        ? `${drive.info.title} (${domain})`
        : domain;
    var forkOf = drive.forkOf
      ? ` ["${drive.forkOf.label}" fork of ${toNiceDomain(drive.forkOf.key)}]`
      : '';
    return html`
      <tr>
        <td>
          <a
            href="hyper://${drive && drive.ident.system ? 'private' : key}/"
            target="_blank"
          >
            ${title} ${forkOf}
          </a>
        ...
        <td>
          <details>
            <summary>
              ${peers.length} ${pluralize(peers.length, 'peer')}
            </summary>
