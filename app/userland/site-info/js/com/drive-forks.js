...
class DriveForks extends LitElement {
  ...
  render() {
    ...
    if (this.info && this.info.forkOf && this.forks.length <= 1) {
      return html`This drive is a fork of
        <a href=${this.info.forkOf}>${toNiceDomain(this.info.forkOf)}</a>`;
    }
    if (!currentFork) {
      return html``;
    }
    return html`
      ...
      ${this.forks.length > 1
        ? html`
            <div class="list">
              ${repeat(this.forks, (fork) => {
                ...
                return html`
                  <a
                    class="fork ${isCurrent ? 'current' : ''}"
                    href=${fork.url}
                    @click=${this.onClickFork}
                  >
                    <div>
                      ${isCurrent
                        ? html`<span class="fas fa-fw fa-caret-right"></span>`
                        : ''}
                      <span class="fork-label"
                        >${fork.forkOf ? fork.forkOf.label : 'Original'}</span
                      >
                    </div>
                    <div>
                      ${fork.forkOf
                        ? html`
                            <button
                              class="transparent"
                              @click=${(e) => this.onClickDelete(e, fork)}
                            >
        ...
        : ''}
      <div>
        <button class="transparent" @click=${this.onClickNewFork}>
          + New Fork
        </button>
        ${currentFork.forkOf
          ? html`
              <button
                class="transparent"
                @click=${(e) => this.onClickDiff(e, currentFork)}
              >
                <span class="diff-merge-icon">◨</span> Diff / Merge Original
              </button>
            `
          : html`
