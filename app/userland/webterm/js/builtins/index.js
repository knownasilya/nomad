          ...
          ${listing
            ...
            .map((entry) => {
              ...
              const mountInfo = entry.stat.mount
                ? html` <a
                    href="hyper://${entry.stat.mount.key}/"
                    ...
                    ></term-icon
                    >${entry.stat.mount.key.slice(
                      0,
                      4
                    )}..${entry.stat.mount.key.slice(-2)})</a
                  >`
                : '';
              const symlinkInfo = entry.stat.linkname
                ? html` <a
                    href=${this.env.resolve(entry.stat.linkname)}
                    class="color-lightgray"
                    style="font-weight: lighter"
                    ><term-icon
                      solid
              ...
              return html`<div>
                <a
                  ...
                  class="color-${color}"
                  ><term-icon icon="${icon}"></term-icon> ${entry.name}</a
                >${mountInfo}${symlinkInfo}
              </div>`;
            })}
        `;
      },
...
export async function ln(opts, target, linkname) {
  ...
  if (targetp.hostname !== linknamep.hostname)
    throw new Error('target and link must be on the same drive');
...
export async function readlink(opts, target) {
  ...
  let st = await targetp.drive
    .stat(targetp.pathname, { lstat: true })
    .catch((e) => undefined);
  if (!st) throw new Error('target does not exist');
...
export async function query(opts = {}, ...paths) {
  ...
  allResults.toHTML = () =>
    html`${allResults.map((r) => html`<a href=${r.url}>${r.path}</a><br />`)}`;
  return allResults;
}
...
export async function cat(opts = {}, location = '') {
  ...
  if (/\.(mp4|webm|mov)$/.test(pathname)) {
    return {
      toHTML: () =>
        html`<video controls><source src=${origin + pathname} /></video>`,
    };
  }
  if (/\.(mp3|ogg)$/.test(pathname)) {
    return {
      toHTML: () =>
        html`<audio controls><source src=${origin + pathname} /></audio>`,
    };
  }
  ...
  async fs_audit_stream(opts = {}) {
    ...
    stream.addEventListener('data', (e) => {
      ...
      html.render(
        html`
          <div>
            <small style="padding: 0 5px; width: 50px; display: inline-block"
              >caller</small
            >
            <a href="${e.detail.caller}">${e.detail.caller}</a>
          </div>
          <div>
            <small style="padding: 0 5px; width: 50px; display: inline-block"
              >target</small
            >
            <a href="hyper://${e.detail.target}/">hyper://${e.detail.target}</a>
          </div>
          <div>
            <small style="padding: 0 5px; width: 50px; display: inline-block"
              >ts</small
