# beaker.autobase as a first-class platform API with Collaborative Drive shape

Multi-writer collaboration is exposed via a new `beaker.autobase` platform API. The primary abstraction is a **Collaborative Drive** — an Autobase-backed drive whose API mirrors `beaker.hyperdrive`. Writer management (`inviteWriter`, `addWriter`) is part of the API, not left to templates or shell UI.
...
- Raw Autobase wrapper (`beaker.autobase.create(bootstrapKey, { apply, open })`): rejected because it requires template authors to understand Autobase internals and diverges from the `beaker.hyperdrive` API shape.
